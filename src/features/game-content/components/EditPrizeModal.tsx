...
interface ModalData {
    handleUpdatePrize: (payload: Record<string, string | number>) => void;
    loading: boolean;
    selectedGame: string;
...
const EditPrizeModal = ({ isOpen }: { isOpen: boolean }) => {
    const { base64, error, convertToBase64 } = useConvertToBase64();
    const { modalData } = useModalContext();

    const { handleUpdatePrize, loading, selectedGame, selectedPrize } = modalData as ModalData;
    ...
    const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        handleUpdatePrize({
            id: selectedPrize?.id as string,
            name,
            quantity,
            image: base64 ? base64 : selectedPrize?.image as string,
            type: selectedGame,
        })
    }
    ...
    useEffect(() => {
        if (selectedPrize && isOpen) {
            setName(selectedPrize.name as string);
            setQuantity(selectedPrize.quantity as string);
        }
    }, [selectedPrize, isOpen]);
    ...
    return (
        <Modal isOpen={isOpen} className="max-w-sm">
            <div className="aspect-square w-full bg-gray-500 rounded-lg relative">
                ...
                {base64 || selectedPrize?.image ? (
                    <Image
                        src={base64 ? base64 : selectedPrize?.image as string}
